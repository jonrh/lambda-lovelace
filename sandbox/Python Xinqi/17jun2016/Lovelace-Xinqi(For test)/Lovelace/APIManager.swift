...
class APIManager {
    ...
    class func getUserTimeline(callback: (JSON)->Void) {
        Alamofire.request(Router.UserTimeline)
    ...
    class func getHomeTimeline(callback: (JSON)->Void) {
        Alamofire.request(Router.HomeTimeline)
            .responseJSON { response in
                guard response.result.error == nil else {
                    print(response.result.error)
                    return
