...
class RecommenderTextual:
    ...
    def get_term_frequency_weightings(self):
        ...
        self.second_net_termfreq_doc = term_frequency_list
        top_terms = []
    ...
    def get_liked_weighting(self, tweet):
        ...
        for word in tweet_text.replace("\n"," ").split():
            ...
            if word.startswith("#"):
                unhashedword = unhashedword[1:]
                #print("*unhashed*" + str(unhashedword) + "*")
            for term in self.termfreq_doc.keys():
                if unhashedword == term:
    ...
    def generate(self, number_of_recommendations, how_many_days_ago):
        ...
        second_net_data_returned = sorted(self.second_net_list, key=self.count_bag, reverse=True)
        #for tweet in data_returned:
        #    for term in termfreq_doc.keys():
        #        if term in tweet['text'].lower():

        prelim_results = data_returned[0:number_of_recommendations]
        print("Original RESULTS!" + str(len(prelim_results)))
        cut_off = 9999999999
        for tweet in prelim_results:
            for term in self.second_net_list:
                if tweet == term:
        ...
        print("RESULTS AFTER BEING CUT!" + str(len(results)))
        print("SECOND NET: " + str(len(self.second_net_list)))
        results.extend(self.second_net_list)
        counts = [self.count_bag(tweet) for tweet in results]
        print("FINALRESULTS!" + str(len(results)))
